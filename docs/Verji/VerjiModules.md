...
**Every secondary trigger we identified during the saga** (a module's `link:` ref to element-web in its `dependencies`; a module's `peerDependencies` semver entry for element-web; etc.) was an *additional entry point* into the same recursive code path. The modules' references made each install slower because they multiplied the number of times yarn entered the recursion. They were not the trigger. The trigger was element-web-v2's own self-reference; the modules' references just amplified it.
...
When element-web-v2 then ran `yarn add file:../verji-news-module`, yarn v1 saw `element-web` in the module's deps and tried to install it **transitively**. Because the current project *is* element-web, this is a circular reference:
...
The prestart's health check skips the wipe when state is healthy, but when state *is* broken, the full wipe of `node_modules/@verji` can still take ~60 seconds on a populated tree. This is pure filesystem I/O at Windows' small-file throughput; no known way to make it faster.
